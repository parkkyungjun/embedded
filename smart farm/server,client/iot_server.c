...
int main(int argc, char *argv[])
{
	...
	CLIENT_INFO client_info[MAX_CLNT] = {{0,-1,"","PKJ_QT","PASSWD"}, \
			 {0,-1,"","PKJ_ARD2","PASSWD"},  {0,-1,"","KSH_LIN","PASSWD"}, \
			 {0,-1,"","PKJ_ARD","PASSWD"},  {0,-1,"","KSH_BLT","PASSWD"}, \
			 {0,-1,"","KSH_SQL","PASSWD"},  {0,-1,"","7","PASSWD"}, \
			 {0,-1,"","FARM1_AD","PASSWD"},  {0,-1,"","9","PASSWD"}, \
			 {0,-1,"","FARM1_CAM","PASSWD"},  {0,-1,"","11","PASSWD"}, \
			 {0,-1,"","FARM1_F1","PASSWD"},  {0,-1,"","13","PASSWD"}, \
			 {0,-1,"","FARM1_F2","PASSWD"},  {0,-1,"","15","PASSWD"}, \
				...
				if(!strcmp(client_info[i].id,pArray[0]))
				{
					...
					//	else
						...
						if(*pArray[0] == '9')
							qt_sock = clnt_sock;
						write(clnt_sock,msg,strlen(msg));
						write(qt_sock,msg,strlen(msg));
					//	write(999, msg,strlen(msg));
...
void * clnt_connection(void *arg)
{
	...
	sprintf(strBuff,"[%s] Disconnect ID (ip:%s,fd:%d,sockcnt:%d)\n",client_info->id,client_info->ip,client_info->fd,clnt_cnt-1);
	write(qt_sock, strBuff,strlen(strBuff));
	log_file(strBuff);
